...
If **Save brief and stop**: write the concepts brief to a working-notes location in the repo (prefer `docs/eg-brainstorms/<slug>-<YYYY-MM-DD>.md` if `docs/` exists, otherwise `notes/eg-brainstorms/<slug>-<YYYY-MM-DD>.md`, otherwise `.eg-brainstorms/<slug>-<YYYY-MM-DD>.md` and add it to .gitignore) ONLY if the user confirms via one more `AskUserQuestion`:
